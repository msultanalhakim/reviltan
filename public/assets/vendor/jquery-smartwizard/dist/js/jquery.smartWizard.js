...
(function (factory) {
  ...
  var SmartWizard = /*#__PURE__*/function () {
    ...
    value: function _showNext() {
        var currentSection = this.current_index; // Get the current section index

        if (currentSection === 0) { // Check if current section is the first section
            if (!validateWizardService()) {
                Swal.fire({
                    title: 'Incomplete Information',
                    text: 'All fields must be filled out before proceeding.',
                    icon: 'error',
                });
                return false; // Prevent showing the next step if validation fails
            }
        } else if (currentSection === 1) { // Check if current section is the second section
            if (!validateWizardTime()) {
              Swal.fire({
                title: 'Incomplete Information',
                text: 'All fields must be filled out before proceeding.',
                icon: 'error',
        ...
        var nextShowableIndex = this._getNextShowable(currentSection);
        if (nextShowableIndex === false) {
            return false;
        }

        this._showStep(nextShowableIndex);
    }
}, {
      key: "_showPrevious",
      ...
      value: function _setButtons(idx) {
        ...
        if (!this.options.cycleSteps) {
          ...
          switch (this._getStepPosition(idx)) {
            ...
            case 'last':
              this.main.find('.sw-btn-next').text('Submit').attr('type', 'button').off('click').on('click', function(e) {
                e.preventDefault();
                Swal.fire({
                  title: 'Confirm Submission',
                  text: "Please verify that all information is correct before submitting.",
                  icon: 'warning',
                  ...
                  if (result.isConfirmed) {
                    self.main.find('form').submit(); // Submit the form if Swal is confirmed
                  }
                });
              });
              break;
          ...
          // If cycling steps is enabled, ensure "Next" is always visible
          this.main.find('.sw-btn-prev').removeClass("disabled");
          this.main.find('.sw-btn-next').removeClass("disabled");
    
          if (this._getStepPosition(idx) === 'last') {
            this.main.find('.sw-btn-next').text('Submit').attr('type', 'button').off('click').on('click', function(e) {
              e.preventDefault();
              Swal.fire({
                title: 'Confirm Submission',
